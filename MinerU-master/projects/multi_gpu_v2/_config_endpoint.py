...
def config_endpoint():
    ...
    if os.environ['MINERU_MODEL_SOURCE'] == 'huggingface':
        try:
            ...
            if response.ok:
                logging.info("Successfully connected to Hugging Face. Using 'huggingface' as model source.")
                return True
            else:
        ...
        try:
            ...
            if response.ok:
                logging.info("Successfully connected to ModelScope. Using 'modelscope' as model source.")
                return True
        except requests.exceptions.RequestException as e:
